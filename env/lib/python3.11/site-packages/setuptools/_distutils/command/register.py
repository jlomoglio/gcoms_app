        ...
        if choice == '1':
            ...
            if code == 200:
                if self.has_config:
                    ...
                    self.announce(
                        (
                            'I can store your PyPI auth so future '
                            'submissions will be faster.'
                        ),
                        log.INFO,
                    )
                    self.announce(
                        '(the auth will be stored in %s)' % self._get_rc_file(),
                        log.INFO,
                    )
                    choice = 'X'
                    while choice.lower() not in 'yn':
                        choice = input('Save your auth (y/N)?')
                        if not choice:
                            choice = 'n'
                    if choice.lower() == 'y':
