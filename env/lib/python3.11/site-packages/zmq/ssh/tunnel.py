...
def _try_passwordless_openssh(server, keyfile):
    """Try passwordless auth with shell ssh command."""
    if pexpect is None:
...
def _try_passwordless_paramiko(server, keyfile):
    """Try passwordless auth with paramiko."""
    if paramiko is None:
...
def openssh_tunnel(
    ...
    keyfile : str; path to private key file
        This specifies a key to be used in ssh auth, default None.
        Regular default ssh keys will be used without specifying this argument.
    password : str;
        Your ssh password to the ssh server. Note that if this is left None,
        you will be prompted for it if passwordless key based auth is unavailable.
