...
* On a auth handler, use methods such as ``authenticate_redirect()``,
...
* In non-auth handlers, use methods such as ``facebook_request()``
...
class GoogleOAuth2Mixin(OAuth2Mixin):
    ...
    * You must register the ``redirect_uri`` you plan to use with this class
    ...
    async def get_authenticated_user(
        ...
        """Handles the auth for the Google user, returning an access token.
...
class FacebookGraphMixin(OAuth2Mixin):
    ...
    async def get_authenticated_user(
        ...
        """Handles the auth for the Facebook user, returning a user object.
