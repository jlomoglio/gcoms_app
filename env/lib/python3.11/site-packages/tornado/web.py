...
class RequestHandler(object):
    ...
    def get_login_url(self) -> str:
        """Override to customize the auth URL based on the request.
...
def authenticated(
    ...
    `auth url <RequestHandler.get_login_url>`.

    If you configure a auth url with a query parameter, Tornado will
    assume you know what you're doing and use it as-is.  If not, it
    will add a `next` parameter so the auth page knows where to send
            ...
            if self.request.method in ("GET", "HEAD"):
                ...
                if "?" not in url:
                    if urllib.parse.urlsplit(url).scheme:
                        # if auth url is absolute, make next absolute too
