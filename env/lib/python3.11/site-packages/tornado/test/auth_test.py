...
class TwitterClientShowUserHandler(TwitterClientHandler):
    @gen.coroutine
    def get(self):
        # TODO: would be nice to go through the auth flow instead of
...
class AuthTest(AsyncHTTPTestCase):
    def get_app(self):
        return Application(
            [
                # test endpoints
                ("/openid/client/auth", OpenIdClientLoginHandler, dict(test=self)),
                (
                    "/oauth10/client/auth",
                ...
                ("/oauth2/client/auth", OAuth2ClientLoginHandler, dict(test=self)),
                ("/facebook/client/auth", FacebookClientLoginHandler, dict(test=self)),
                ("/twitter/client/auth", TwitterClientLoginHandler, dict(test=self)),
                (
    ...
    def test_openid_redirect(self):
        response = self.fetch("/openid/client/auth", follow_redirects=False)
        self.assertEqual(response.code, 302)
        self.assertTrue("/openid/server/authenticate?" in response.headers["Location"])

    def test_openid_get_user(self):
        response = self.fetch(
            "/openid/client/auth?openid.mode=blah"
    ...
    def test_oauth10_redirect(self):
        response = self.fetch("/oauth10/client/auth", follow_redirects=False)
        self.assertEqual(response.code, 302)
        self.assertTrue(
    ...
    def test_oauth10_get_user(self):
        response = self.fetch(
            "/oauth10/client/auth?oauth_token=zxcv",
    ...
    def test_oauth10a_redirect(self):
        response = self.fetch("/oauth10a/client/auth", follow_redirects=False)
        self.assertEqual(response.code, 302)
        self.assertTrue(
            response.headers["Location"].endswith(
                "/oauth1/server/authorize?oauth_token=zxcv"
    ...
    def test_oauth10a_get_user(self):
        response = self.fetch(
            "/oauth10a/client/auth?oauth_token=zxcv",
            headers={"Cookie": "_oauth_request_token=enhjdg==|MTIzNA=="},
        )
        response.rethrow()
        parsed = json_decode(response.body)
    ...
    def test_oauth2_redirect(self):
        response = self.fetch("/oauth2/client/auth", follow_redirects=False)
        self.assertEqual(response.code, 302)
        self.assertTrue("/oauth2/server/authorize?" in response.headers["Location"])

    def test_facebook_login(self):
        response = self.fetch("/facebook/client/auth", follow_redirects=False)
        self.assertEqual(response.code, 302)
        self.assertTrue("/facebook/server/authorize?" in response.headers["Location"])
        response = self.fetch(
            "/facebook/client/auth?code=1234", follow_redirects=False
        )
    ...
    def test_twitter_redirect(self):
        self.base_twitter_redirect("/twitter/client/auth")
    ...
    def test_twitter_get_user(self):
        response = self.fetch(
            "/twitter/client/auth?oauth_token=zxcv",
            headers={"Cookie": "_oauth_request_token=enhjdg==|MTIzNA=="},
        )
        response.rethrow()
        parsed = json_decode(response.body)
...
class GoogleLoginHandler(RequestHandler, GoogleOAuth2Mixin):
    def initialize(self, test):
        self.test = test
        self._OAUTH_REDIRECT_URI = test.get_url("/client/auth")
...
class GoogleOAuth2Test(AsyncHTTPTestCase):
    def get_app(self):
        return Application(
            [
                # test endpoints
                ("/client/auth", GoogleLoginHandler, dict(test=self)),
    ...
    def test_google_login(self):
        response = self.fetch("/client/auth")
